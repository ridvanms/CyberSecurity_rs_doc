...
Nmap can detect the Operating System (OS) based on its behaviour and any telltale signs in its responses. OS detection can be enabled using `-O`; this is an _uppercase O_ as in OS. In this example, we ran `nmap -sS -O 10.10.248.112` on the AttackBox. Nmap detected the OS to be Linux 3.X, and then it guessed further that it was running kernel 3.13.

Pentester Terminal

```shell-session
pentester@TryHackMe$ sudo nmap -sS -O 10.10.248.112

Starting Nmap 7.60 ( https://nmap.org ) at 2021-09-10 05:04 BST
Nmap scan report for 10.10.248.112
...
In the following example, we executed `nmap -sS --traceroute 10.10.248.112` on the AttackBox. We can see that there are no routers/hops between the two as they are connected directly.

Pentester Terminal

```shell-session
pentester@TryHackMe$ sudo nmap -sS --traceroute 10.10.248.112
...
Some scripts belong to more than one category. Moreover, some scripts launch brute-force attacks against services, while others launch DoS attacks and exploit systems. Hence(следователно), it is crucial to be careful when selecting scripts to run if you don’t want to crash services or exploit them.
...
# Nmap 7.60 scan initiated Fri Sep 10 05:14:19 2021 as: nmap -sS -sV -O -oN MACHINE_IP_scan 10.10.236.228
Nmap scan report for 10.10.236.228
...
#! You need to ensure you are using the correct command to identify service versions on open ports as outlined in the objectives.
- Going forward, try: `nmap -sV --version-light MACHINE_IP` to detect versions of running services.

#! You need to run the OS detection command to fulfill the objective of identifying the OS based on any signs revealed by the target. 
- In future environments, execute: `nmap -O MACHINE_IP` to detect the operating system.

#! Utilizing Nmap’s scripting capabilities can enrich your enumeration and is a key objective in this room.
- To enrich enumeration, use: `nmap -sS -sC MACHINE_IP` for default scripts.