...
On your AttackBox terminal, you can start to use ping as `ping 10.10.147.65` or `ping HOSTNAME`. In the latter, the system needs to resolve HOSTNAME to an IP address before sending the ping packet. If you don’t specify the count on a Linux system, you will need to hit `CTRL+c` to force it to stop. Hence, you might consider `ping -c 10 10.10.147.65` if you just want to send ten packets. This is equivalent to `ping -n 10 10.10.147.65` on a MS Windows system.
...
In the following example, we have specified the total count of packets to 5. From the AttackBox’s terminal, we started pinging `10.10.147.65`. We learned that `10.10.147.65` is up and is not blocking ICMP echo requests. Moreover, any firewalls and routers on the packet route are not blocking ICMP echo requests either.

AttackBox Terminal

```shell-session
user@AttackBox$ ping -c 5 10.10.147.65
PING 10.10.147.65 (10.10.147.65) 56(84) bytes of data.
64 bytes from 10.10.147.65: icmp_seq=1 ttl=64 time=0.636 ms
64 bytes from 10.10.147.65: icmp_seq=2 ttl=64 time=0.483 ms
64 bytes from 10.10.147.65: icmp_seq=3 ttl=64 time=0.396 ms
64 bytes from 10.10.147.65: icmp_seq=4 ttl=64 time=0.416 ms
64 bytes from 10.10.147.65: icmp_seq=5 ttl=64 time=0.445 ms

--- 10.10.147.65 ping statistics ---
...
Let’s consider the following case: we shut down the target virtual machine and then tried to ping `10.10.147.65`. As you would expect in the following example, we don’t receive any reply.
...
On Linux and macOS, the command to use is `traceroute 10.10.147.65`, and on MS Windows, it is `tracert 10.10.147.65`. `traceroute` tries to discover the routers across the path from your system to the target system.
...
However, the telnet client, with its simplicity, can be used for other purposes. Knowing that telnet client relies on the TCP protocol, you can use Telnet to connect to any service and grab its banner. Using `telnet 10.10.147.65 PORT`, you can connect to any service running on TCP and even exchange a few messages unless it uses encryption.
...
```shell-session
pentester@TryHackMe$ telnet 10.10.147.65 80
Trying 10.10.147.65...
Connected to 10.10.147.65.
Escape character is '^]'.
GET / HTTP/1.1
host: telnet
...
First, you can connect to a server, as you did with Telnet, to collect its banner using `nc 10.10.147.65 PORT`, which is quite similar to our previous `telnet 10.10.147.65 PORT`. Note that you might need to press SHIFT+ENTER after the GET line.

Pentester Terminal

```shell-session
pentester@TryHackMe$ nc 10.10.147.65 80
GET / HTTP/1.1
host: netcat
...
In the terminal shown above, we used netcat to connect to 10.10.147.65 port 80 using `nc 10.10.147.65 80`. Next, we issued a get for the default page using `GET / HTTP/1.1`; we are specifying to the target server that our client supports HTTP version 1.1. Finally, we need to give a name to our host, so we added on a new line, `host: netcat`; you can name your host anything as this has no impact on this exercise.
...
On the _client_-side, you would issue `nc 10.10.147.65 PORT_NUMBER`. Here is an example of using `nc` to echo. After you successfully establish a connection to the server, whatever you type on the client-side will be echoed on the server-side and vice versa.

Consider the following example. On the server-side, we will listen on port 1234. We can achieve this with the command `nc -vnlp 1234` (same as `nc -lvnp 1234`). In our case, the listening server has the IP address `10.10.147.65`, so we can connect to it from the client-side by executing `nc 10.10.147.65 1234`. This setup would echo whatever you type on one side to the other side of the TCP tunnel. You can find a recording of the process below. Note that the listening server is on the left side of the screen.
...
|Command|Example|
|---|---|
|ping|`ping -c 10 10.10.148.81` on Linux or macOS|
|ping|`ping -n 10 10.10.148.81` on MS Windows|
|traceroute|`traceroute 10.10.148.81` on Linux or macOS|
|tracert|`tracert 10.10.148.81` on MS Windows|
|telnet|`telnet 10.10.148.81 PORT_NUMBER`|
|netcat as client|`nc 10.10.148.81 PORT_NUMBER`|
