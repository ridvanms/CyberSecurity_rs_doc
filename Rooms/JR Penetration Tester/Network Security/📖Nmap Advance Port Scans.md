...
> In the context of spoofing, the term 'SPOOFED_IP' refers to a falsified source IP address that an attacker uses when sending packets to a target machine. When the attacker sends packets with this spoofed IP address, the target machine believes that the packets originated from this address


In some network setups, you will be able to scan a target system using a spoofed IP address and even a spoofed MAC address. Such a scan is only beneficial in a situation where you can guarantee to capture the response. If you try to scan a target from some random network using a spoofed IP address, chances are you won’t have any response routed to you, and the scan results could be unreliable.

The following figure shows the attacker launching the command `nmap -S SPOOFED_IP 10.10.148.158`. Consequently, Nmap will craft all the packets using the provided source IP address `SPOOFED_IP`. The target machine will respond to the incoming packets sending the replies to the destination IP address `SPOOFED_IP`. For this scan to work and give accurate results, the attacker needs to monitor the network traffic to analyze the replies.
...
In general, you expect to specify the network interface using `-e` and to explicitly disable ping scan `-Pn`. Therefore, instead of `nmap -S SPOOFED_IP 10.10.148.158`, you will need to issue `nmap -e NET_INTERFACE -Pn -S SPOOFED_IP 10.10.148.158` to tell Nmap explicitly which network interface to use and not to expect to receive a ping reply. It is worth repeating that this scan will be useless if the attacker system cannot monitor the network for responses.
...
You can launch a decoy scan by specifying a specific or random IP address after `-D`. For example, `nmap -D 10.10.0.1,10.10.0.2,ME 10.10.148.158` will make the scan of 10.10.148.158 appear as coming from the IP addresses 10.10.0.1, 10.10.0.2, and then `ME` to indicate that your IP address should appear in the third order. Another example command would be `nmap -D 10.10.0.1,10.10.0.2,RND,RND,ME 10.10.148.158`, where the third and fourth source IP addresses are assigned randomly, while the fifth source is going to be the attacker’s IP address. In other words, each time you execute the latter command, you would expect two new random IP addresses to be the third and fourth decoy sources.


> [!decoys] decoys
> In network security, decoys are used by attackers to obscure their true location and confuse monitoring systems. When an attacker scans a target using spoofing techniques, it can sometimes be detected if the target or security systems are vigilant.
...
|---|---|
|TCP Null Scan|`sudo nmap -sN 10.10.155.175`|
|TCP FIN Scan|`sudo nmap -sF 10.10.155.175`|
|TCP Xmas Scan|`sudo nmap -sX 10.10.155.175`|
|TCP Maimon Scan|`sudo nmap -sM 10.10.155.175`|
|TCP ACK Scan|`sudo nmap -sA 10.10.155.175`|
|TCP Window Scan|`sudo nmap -sW 10.10.155.175`|
|Custom TCP Scan|`sudo nmap --scanflags URGACKPSHRSTSYNFIN 10.10.155.175`|
|Spoofed Source IP|`sudo nmap -S SPOOFED_IP 10.10.155.175`|
|Spoofed MAC Address|`--spoof-mac SPOOFED_MAC`|
|Decoy Scan|`nmap -D DECOY_IP,ME 10.10.155.175`|
|Idle (Zombie) Scan|`sudo nmap -sI ZOMBIE_IP 10.10.155.175`|
