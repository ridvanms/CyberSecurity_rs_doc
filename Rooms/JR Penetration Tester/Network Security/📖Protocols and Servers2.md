...
```shell-session
pentester@TryHackMe$ telnet 10.10.77.96 110
Trying 10.10.77.96...
Connected to 10.10.77.96.
Escape character is '^]'.
+OK 10.10.77.96 Mail Server POP3 Wed, 15 Sep 2021 11:05:34 +0300 
USER frank
+OK frank
PASS D2xc9CgD
...
.
QUIT
+OK 10.10.77.96 closing connection
...
- `hydra -l mark -P /usr/share/wordlists/rockyou.txt 10.10.77.96 ftp` will use `mark` as the username as it iterates over the provided passwords against the FTP server.
- `hydra -l mark -P /usr/share/wordlists/rockyou.txt ftp://10.10.77.96` is identical to the previous example. `10.10.77.96 ftp` is the same as `ftp://10.10.77.96`.
- `hydra -l frank -P /usr/share/wordlists/rockyou.txt 10.10.77.96 ssh` will use `frank` as the user name as it tries to login via SSH using the different passwords.
